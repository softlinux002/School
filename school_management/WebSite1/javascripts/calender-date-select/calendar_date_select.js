...
Date.prototype.toFormattedString = function(include_time,formatted){
  var hour, str;
  formatted = formatted || false;
  if(formatted){
    switch(Date.date_format){
      case 2:
          str = Date.months[this.getMonth()] + " " + format_digits(this.getDate()) + ", " + this.getFullYear();
          break;
      case 3:
          str = this.getFullYear() + " " + Date.months[this.getMonth()] + " " + format_digits(this.getDate());
          break;
      default:
          str = format_digits(this.getDate()) + " " + Date.months[this.getMonth()] + " " + this.getFullYear();
          break;
    }
    ...
    str = this.getFullYear() + "-" + (format_digits(this.getMonth()+1)) + "-" + format_digits(this.getDate());
    if (include_time) { hour = this.getHours(); str += " " + this.getAMPMHour() + ":" + this.getPaddedMinutes() + " " + this.getAMPM() }    
    return str;
  }
  
}
function format_digits(num){
...
CalendarDateSelect.prototype = {
  initialize: function(target_element, options) {
    ...
    if(options.locale_months !== undefined){
        Date.months = options.locale_months.split(',');
    }
    if(options.date_format != undefined){
        Date.date_format = options.date_format;
    }
    global_val = options;
  ...
  dayHover: function(element) {
    ...
    this.updateFooter(hover_date.toFormattedString(this.use_time,true));
  },
  ...
  dateString: function(formatted) {
      formatted = formatted || false;
    return (this.selection_made) ? this.selected_date.toFormattedString(this.use_time,formatted) : "&#160;";
  },
  parseDate: function()
  {
  ...
  updateValue: function() {
    var last_value = this.target_element.value;
    this.target_element.value = this.dateString(false);
    if(this.target_element.nextSibling == null){
       this.target_element.ancestors()[0].nextSibling.value = this.dateString(true);
   }else{
       this.target_element.nextSibling.value = this.dateString(true);
   }
    if (last_value!=this.target_element.value) this.callback("onchange");
  },
  today: function(now) {
